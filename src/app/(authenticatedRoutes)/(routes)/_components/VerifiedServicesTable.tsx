...
const VerifiedServicesTable: FC<ServicesTableProps> = () => {
  const [rowSelection, setRowSelection] = useState({});
  const [sorting, setSorting] = useState<SortingState>([]);
  const [data, setData] = useState<IService[]>([]);
  const services = useSelector(
    (state: RootState) => state.service.verifiedServices
  );
    ...
    <div className="h-[60vh] px-4 bg-white relative rounded-lg overflow-y-auto">
      {loading ? (
        ...
        <h3 className="flex justify-center text-sm text-slate-500 h-full items-center">
          Currently, No verified service(s)
        </h3>
      )}
    </div>
  );
...
export default memo(VerifiedServicesTable);
