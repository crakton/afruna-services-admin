...
const ReportTable = () => {
  ...
  const columns = useMemo<ColumnDef<T_Report>[]>(
    () => [
      {
        accessorKey: "id",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div key={row.id} className="flex gap-2 items-center ml-8 ">
            ...
            Lativari dress dress dress
            </span>
          </div>
        ),
        header: () => (
          <span className="text-sm text-[#7C7C7C] ml-8 ">Provider</span>
        ),
      },
      {
        accessorKey: "user",
        cell: ({ row }) => (
          <div key={row.id} className="flex gap-2 items-center ml-8 mr-8">
            <Image
              src={imgs.provider3}
              alt={'user'}
              width={35}
        ...
        header: () => <span className="text-sm text-[#7C7C7C] ml-8">Reported by User</span>,
      },
      {
        accessorKey: "description",
        cell: ({ row }) => (
          <div key={row.id} className="flex gap-4 items-center ml-8">
            <span className=" text-slate-600 text-xs">{row.original.description}</span>
          </div>
        ),
        header: () => (
          <span className="text-sm text-[#7C7C7C] ml-8">Description</span>
        ),
      },
      {
        id: "actions",
        cell: ({ row }) => (
          <div className="flex justify-start gap-1 items-center ml-8">
        ...
        header: () => <span className="text-sm text-[#7C7C7C] ml-8">Action</span>,
      },
    ],
    [data]
  );
                  ...
                  {headerGroup.headers.map((header) => (
                    ...
                    >
                      {header.index > 0 && header.id !== "actions" ? (
                        <span className="flex min-w-[13rem] pr-4 justify-between gap-2 items-center w-fit">
                          {flexRender(
