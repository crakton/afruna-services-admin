  ...
  const columns = useMemo<ColumnDef<T_Providers>[]>(
    () => [
        ...
        cell: ({ row }) => (
          <div className="flex justify-start items-center gap-1">
            <div className="relative overflow-hidden rounded-full w-[35px] h-[35px] flex justify-center items-center">
              ...
              ) : <div className="w-full h-full flex justify-center items-center bg-slate-200 text-xs">{`${row.original.firstName.charAt(0).toUpperCase()} ${row.original.lastName.charAt(0).toUpperCase()}`}</div>}
            </div>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-afruna-blue text-xs">{row.original.phoneNumber}</span>
        ),
        header: () => <span className="text-sm text-[#7C7C7C]">Mobile Number</span>,
      },
      {
        accessorKey: "date",
        cell: ({ row }) => {
