...
interface CompletedBookingsTableProps {
  // bookings: any[]
}

const CompletedBookingsTable: FC<CompletedBookingsTableProps> = () => {
  const loading = useSelector((state: RootState) => state.loading.loading);
  const [rowSelection, setRowSelection] = useState({});
  const [data, setData] = useState<T_Bookings[]>([]);
  const [sorting, setSorting] = useState<SortingState>([]);
  const bookings = useSelector((state: RootState) => state.booking.completedBookings);
      ...
      <div className="h-[65vh] px-4 bg-white overflow-auto relative w-full rounded-xl border shadow-sm border-slate-300">
        {loading ? (
          ...
          <h3 className="flex justify-center text-sm text-slate-500 h-full items-center">
            Currently, No Client as compieted bookings yet
          </h3>
        )}
      </div>
    </div>
...
export default memo(CompletedBookingsTable);
